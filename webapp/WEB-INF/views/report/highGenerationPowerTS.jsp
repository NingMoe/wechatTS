...
<body>
	...
	<div class="am-alert am-text-lg am-text-center" style="padding:2px;margin-bottom:0px;">按发电次数统计TOP10基站</div>
	  ...
	  <table  class="am-table am-table-bordered am-table-striped am-table-hover am-text-center am-text-nowrap">
	    <thead>
		  <tr>
		    ...
		    <th style="text-align:center;">发电次数</th>
	    ...
	    <tbody id="highGenerationNumList">
		  <c:forEach var="obj" items="${highGenerationNumList}" varStatus="status">
		  	<tr id="${obj.highGeneratepowerId}" onclick="responseText('${obj.highGeneratepowerId}','${obj.reason}','${obj.statu}',this);">
		  		<td>
		  			<button id ="response_${obj.highGeneratepowerId}" style="border:none;"><i class="am-icon-pencil-square-o" ></i></button>
		  		</td>
			    <td>${obj.county}</td>
	...
	<div class="am-alert am-text-lg am-text-center" style="padding:2px;margin-bottom:0px;">按发电时长统计TOP10基站</div>
	<div class="am-scrollable-horizontal">
	  <table  class="am-table am-table-bordered am-table-striped am-table-hover am-text-center am-text-nowrap">
	    <thead>
		  <tr>
		    ...
		    <th style="text-align:center;">区县</th>
		    <th style="text-align:center;">基站名称</th>
		    <th style="text-align:center;">发电时长(分)</th>
		    <th style="text-align:center;">是否解决</th>
		    <th style="text-align:center;">问题原因</th>
		    <th style="text-align:center;">代维公司</th>
		    <th style="text-align:center;">基站编码</th>
	    ...
	    <tbody id="highGenerationLengthList">
		  <c:forEach var="obj" items="${highGenerationLengthList}" varStatus="status">
		  	<tr id="${obj.highGeneratepowerId}" onclick="responseText('${obj.highGeneratepowerId}','${obj.reason}','${obj.statu}',this);">
		  		<td>
		  			<button id ="response_${obj.highGeneratepowerId}" style="border:none;"><i class="am-icon-pencil-square-o" ></i></button>
		  		</td>
	...
	function responseText(id,reasonVal,statuVal,el){
		...
		$('#response-prompt').modal({
		      ...
		      onConfirm: function(e) {
		    	  ...
		    	  $.ajax({type:"POST",dataType:"json",url:'${ctx}/fourHight/updateHighGenerationTS.json',
		  			data:{
		  				highGeneratepowerId:idVal,
		  				reason:reason,
		  				statu:statu
		  			},
