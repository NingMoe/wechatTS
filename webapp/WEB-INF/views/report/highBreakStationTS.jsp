...
<body>
	...
	<div class="am-alert am-text-lg am-text-center" style="padding:2px;margin-bottom:0px;">按断站次数统计TOP10基站</div>
	  ...
	  <table  class="am-table am-table-bordered am-table-striped am-table-hover am-text-center am-text-nowrap">
	    <thead>
		  <tr>
		    ...
		    <th style="text-align:center;">断站次数</th>
	    ...
	    <tbody id="highBreakStationNumList">
		  <c:forEach var="obj" items="${highBreakStationNumList}" varStatus="status">
		  	<tr id="${obj.highBreakStationId}" onclick="responseText('${obj.highBreakStationId}','${obj.reason}','${obj.statu}',this);">
		  		<td>
		  			<button id ="response_${obj.highBreakStationId}" style="border:none;" ><i class="am-icon-pencil-square-o" ></i></button>
		  		</td>
			    <td>${obj.county}</td>
	...
	<div class="am-alert am-text-lg am-text-center" style="padding:2px;margin-bottom:0px;">按断站时长统计TOP10基站</div>
	<div class="am-scrollable-horizontal">
	  <table  class="am-table am-table-bordered am-table-striped am-table-hover am-text-center am-text-nowrap">
	    <thead>
		  <tr>
		    ...
		    <th style="text-align:center;">区县</th>
		    <th style="text-align:center;">基站名称</th>
		    <th style="text-align:center;">断站时长(分)</th>
		    <th style="text-align:center;">是否解决</th>
		    <th style="text-align:center;">问题原因</th>
		    <th style="text-align:center;">代维公司</th>
		    <th style="text-align:center;">基站编码</th>
	    ...
	    <tbody id="highBreakStationLengthList">
		  <c:forEach var="obj" items="${highBreakStationLengthList}" varStatus="status">
		  	<tr id="${obj.highBreakStationId}" onclick="responseText('${obj.highBreakStationId}','${obj.reason}','${obj.statu}',this);">
		  		<td>
		  			<button id ="response_${obj.highBreakStationId}" style="border:none;"><i class="am-icon-pencil-square-o" ></i></button>
		  		</td>
	...
	function responseText(id,reasonVal,statuVal,el){
		...
		$('#response-prompt').modal({
		      ...
		      onConfirm: function(e) {
		    	  ...
		    	  $.ajax({type:"POST",dataType:"json",url:'${ctx}/fourHight/updateHighBreakStationTS.json',
		  			data:{
		  				highBreakStationId:idVal,
		  				reason:reason,
		  				statu:statu
		  			},
