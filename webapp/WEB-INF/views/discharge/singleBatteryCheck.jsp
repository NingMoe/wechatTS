...
<body>
	...
	<div id="dischargeContainer" >
		...
		<div class="app-tabs-bd" style="margin-bottom: 0px">
			<form id="formContainer" class="am-form" action="${ctx}/discharge/saveDischargeRecordStepTwo" method="post">
				<c:forEach items="${turntimeList}" var="turn">
					<c:forEach items="${checkRecordMap}" var="map">
						<c:if test="${map.key eq turn}">
		...
		<div class="app-div-btn-tool">
			<button onclick="submitNext()" type="button" class="am-btn am-btn-success am-radius am-btn-block">
				<strong>下一步</strong>
			</button>
		</div>
...
!function(t) {
	...
	function saveBatteryV(obj,group_id,turn_num,check_num){
		var v = $(obj).val();
		var url = "${ctx}/singleBatteryCheck/saveBatteryCheckRecord?groupId="+group_id+"&turnNum="+turn_num+"&checkNum="+check_num+"&voltage="+v;
	...
	function saveTerminalV(obj,group_id,turn_num){
		var v = $(obj).val();
		var url = "${ctx}/singleBatteryCheck/saveTerminalVoltage?groupId="+group_id+"&turnNum="+turn_num+"&voltage="+v;
		$.get(url,function(data,status){
			if(status=="success"){
				//nothing
			}
	...
	function submitNext(){
		showLoading("正在提交中...");
		var url="${ctx}/discharge/checkBatteryCheckStatus.json";
		$.get(url,function(json) {
			closeLoading();
			if (json.status == "0") {
				window.location.href="${ctx}/singleBatteryCheck/saveBatteryCheckStepTwo";
			} else {
				modalAlert(json.status);
			}
		}, "json");
	}
